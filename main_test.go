...
func TestBrokerConnection_noTLS(t *testing.T) {
	...
	srv := startServer(t, 5673)
	defer srv.Close()
	// errs := make(chan error, 1)
	go func() {
		// select {
		// case <-time.After(10 * time.Millisecond):
		// 	err := fmt.Errorf("server timeout waiting for handshake from client")
		// 	errs <- err
		session := <-srv.Sessions
	...
	// if err := <-errs; err != nil {
	// 	t.Fatalf("go routine returned error: %v", err)
	// }

	viper.Set("broker.ssl", "false")
	viper.Set("broker.port", "5673")
	assert.NotPanics(t, func() { brokerConnection() })
}

func TestBrokerConnection_TLS(t *testing.T) {
	fmt.Println("Test main.brokerConnection with TLS")
	viper.Reset()
	viper.Set("server.confFile", "dev_utils/config.yaml")
	initialization()

	srv := startTLSServer(t, 5674, tlsServerConfig())
	defer srv.Close()
	// errs := make(chan error, 1)
	go func() {
		// select {
		// case <-time.After(10 * time.Millisecond):
		// 	err := fmt.Errorf("server timeout waiting for TLS handshake from client")
		// 	errs <- err
		session := <-srv.Sessions
		session.connectionOpen()
		session.connectionClose()
...
func defaultConfig() amqp.Config {
	return amqp.Config{
		SASL:   []amqp.Authentication{&amqp.PlainAuth{"guest", "guest"}},
		Vhost:  "/",
		Locale: "en_US",
	}
}
...
func tlsServerConfig() *tls.Config {
	...
	if ca, err := ioutil.ReadFile("dev_utils/certs/ca.crt"); err == nil {
		cfg.ClientCAs.AppendCertsFromPEM(ca)
	} else {
		fmt.Printf("caLoad: %v", err)
	}
	if cert, err := tls.LoadX509KeyPair("dev_utils/certs/mq.crt", "dev_utils/certs/mq.key"); err == nil {
