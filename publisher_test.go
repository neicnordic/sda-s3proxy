...
func TestDialTLS(t *testing.T) {
	srv := startTLSServer(t, 5001, tlsServerConfig())
	defer srv.Close()
	go func() {
		select {
	...
	amqps := "amqps://mquser:mqpass@localhost:5001/vhost"
	cfg := new(tls.Config)
	cfg.RootCAs = x509.NewCertPool()
	if ca, err := ioutil.ReadFile("dev_utils/certs/ca.crt"); err == nil {
		cfg.RootCAs.AppendCertsFromPEM(ca)
	}
	if cert, err := tls.LoadX509KeyPair("dev_utils/certs/client.crt", "dev_utils/certs/client.key"); err == nil {
		cfg.Certificates = append(cfg.Certificates, cert)
	}
	c, err := DialTLS(amqps, cfg)
	assert.NoError(t, err)
	c.Close()
}

func TestDial(t *testing.T) {
	srv := startServer(t, 5002)
	defer srv.Close()
	go func() {
		select {
		case <-time.After(10 * time.Millisecond):
			t.Fatalf("server timeout waiting for handshake from client")
		case session := <-srv.Sessions:
			session.connectionOpen()
			session.connectionClose()
			session.S.Close()
	...
	amqp := "amqp://mquser:mqpass@localhost:5002/vhost"
